...
def read_rules():
    ...
    desc = {}
    tag = {}
    dom_tree = xml.dom.minidom.parse(RULES_FILE)
...
def is_valid_node(filename, file_path):
    ...
    # Files to be Scanned
    scan_file_extensions = SCAN_FILES_EXTENSION
    ...
    if valid:
        data = unicode_safe_file_read(file_path)
...
def general_code_analysis(paths):
    ...
    try:
            ...
            for root, _, files in os.walk(path):
                for filename in files:
                    ...
                    all_files.append({relative_path.replace(
                        "/", "", 1): full_file_path.replace(UPLOAD_FOLDER, "", 1)})
                    ...
                    if data is not None:
                        ...
                        for line_no, line in enumerate(lines):
                            ...
                            if not line.lstrip().startswith("//") and not line.lstrip().startswith("/*"):
                                ...
                                for rule_key in dict.keys(scan_rules["vuln_rules"]):
                                ...
                                for mulregex in dict.keys(scan_rules["vuln_mul_regex"]):
                                    ...
                                    if re.search(sig_source, data):
                                        if re.search(sig_line, line):
                                            ...
                                            security_issues.append(finding)
                                # Dynamic Regex
                                for dynregex in dict.keys(scan_rules["vuln_dyn_regex"]):
                                            ...
                                            for line_no, nline in enumerate(lines):
                                                if re.search(dyn_sig, nline):
                                                    ...
                                                    finding[
                                                        "filename"] = filename
                                                    finding[
                                                        "path"] = full_file_path.replace(UPLOAD_FOLDER, "", 1)
                                                    finding["sha2"] = gen_sha256_hash(
                                                        finding["lines"].encode(encoding="utf-8", errors="replace"))
        ...
        print_exception("Error Performing Code Analysis")


paths = []
paths.append("./../../Modified_Kodi_JS")
general_code_analysis(paths);